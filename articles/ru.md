...
Чтобы создать страницу в Gatsby, достаточно просто поместить новый файл в папку _/src/pages_, и он будет скомпилирован в отдельную HTML-страницу. **Важно заметить, что путь к этой странице будет соответствовать фактическому пути с названием**. Например, добавим еще несколько страниц:
...
Как известно, в любом проекте всегда есть повторяющиеся элементы, для вебсайтов это хедер, футер, навигационная панель. Также страницы, вне зависимости от контента, строятся по опрделенной структуре, и так как **Gatsby** это компилятор для **React**, здесь используется тот же компонентный подход для решения этих проблем.
